...
Data_fine$foraging_signs_bool <- (Data_fine$foraging %in% c(1,2)) | (Data_fine$eating_second_half %in% c(1,2))

dive_sums <- Data_fine %>% 
              group_by(divenum) %>%
              summarise(num_forg = sum(foraging_signs_bool),
