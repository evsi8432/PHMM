...
if(K == 1){
  ...
  data <- expand.grid(X=states, 
                      Y=rev(states))
  
  temp <- t(hmm$mle$gamma)
  for(i in 1:6){temp[i,i] <- NA}
  temp0 <- matrix(0,nrow = 6,ncol = 6)
  for(i in 1:6){temp0[,i] <- temp[,7-i]}
  
  data$Z <- expand.grid(temp0)$Var1
  
  plot0 <- ggplot(data, aes(x = X, y = Y, fill = Z)) + 
    geom_tile() + 
  ...
  ggsave(paste0(directory,"/plt/Gamma_",
                paste0(setdiff(names(dist),"knownState"),collapse = "_"),"_",
                round(log10(lambda),3),"_",
                K,".png"), 
         plot0, 
