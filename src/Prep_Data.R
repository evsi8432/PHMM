...
Prep_Data <- function(whale){
  ...
  spacing = 25
  rawDataDown <- rawData[seq(1,nrow(rawData),spacing),]
  TDRData0 <- createTDR(rawDataDown$Time, rawDataDown$p,
                        dtime = spacing*0.02, file="temp.csv")
  
  # use 1 m threshold
  TDRData <- calibrateDepth(TDRData0, zoc.method = "offset", offset=0,
                            dive.thr=1)
  
  diveSums <- diveStats(TDRData)
  
  diveSums$endasc <- diveSums$begdesc + diveSums$divetim
  
  divestarts <- diveSums$begdesc
  bottstarts <- diveSums$enddesc
  bottends <- diveSums$begasc
  diveends <- diveSums$endasc
  
  write.csv(diveSums,file=paste0("../dat/diary/",whale,"/diveSummary",
                                 whale,".csv"))
