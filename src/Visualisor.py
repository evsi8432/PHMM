...
class Visualisor:
    ...
    def plot_dive_features(self,sdive,edive,df_cols,data_cols,file=None,ylabs=None,nrows=None):
        ...
        plt.subplots(nrows,1,figsize=(12,3*nrows))
        fignum = 1
        ...
        for i,col in enumerate(df_cols):
            ...
            if 'subdive_state_' not in col:
                ...
                for state,dive_df in enumerate(dives):
                    ...
                    plt.plot(dive_df['time'],dive_df[col],
                             '.',color=colors[state],markersize=4)
                if 'prob' in col:
                    plt.plot(dive[dive[col] > -0.01]['time'],dive[dive[col] > -0.01][col],
                    ...
                    plt.plot(dive['time'],dive[col],'k--',alpha=0.5,linewidth=0.5)
                plt.yticks(fontsize=10)
                ...
                for dive_num in range(sdive+1,edive+1):
                    vline = min(dive[dive['dive_num'] == dive_num]['time'])
            ...
            if 'dive_state_' not in col or 'subdive_state' in col:
                ...
                for state,subdive_df in enumerate(subdives):
                    ...
                    #         '.',color=colors[state],markersize=4)
                    plt.plot(subdive_df['time'],subdive_df[col],
                             '.',color=colors[state],markersize=4)
                if 'prob' in col:
                    plt.plot(dive[dive[col] > -0.01]['time'],
